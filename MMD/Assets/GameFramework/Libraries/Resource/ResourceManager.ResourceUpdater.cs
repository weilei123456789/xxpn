...
namespace GameFramework.Resource
{
    internal sealed partial class ResourceManager : GameFrameworkModule, IResourceManager
    {
        ...
        private sealed partial class ResourceUpdater
        {
            ...
            private bool m_IsContainCourseware;
                ...
                if (m_UpdateAllowed && !m_UpdateComplete)
                {
                    if (m_UpdateWaitingInfo.Count > 0)
                    {
                        if (m_DownloadManager.FreeAgentCount > 0)
                        {
                            ...
                            if (!m_IsContainCourseware)
                            {
                                if (!updateInfo.ResourceName.Name.Contains(m_ResourceManager.CoursewareSearchPath))
                                {
            ...
            public void UpdateCoursewareResources(CoursewareUpdate[] coursewareUpdates)
            {
                if (m_DownloadManager == null)
                {
                    throw new GameFrameworkException("You must set download manager first.");
                ...
                for (int i = 0; i < m_ResourceManager.CoursewareUpdate.Count; i++)
                {
                    CoursewareUpdate find = m_ResourceManager.CoursewareUpdate[i];
                    for (int j = 0; j < coursewareUpdates.Length; j++)
                    {
                        if (find == coursewareUpdates[j])
                        {
                            if (coursewareUpdates[j].IsNeedUpdate)
                            {
                                AddResourceUpdate(new ResourceName(coursewareUpdates[j].ResourceName.Name, coursewareUpdates[j].ResourceName.Variant),
                                  (LoadType)coursewareUpdates[j].LoadType, coursewareUpdates[j].Length, coursewareUpdates[j].HashCode, coursewareUpdates[j].ZipLength, coursewareUpdates[j].ZipHashCode, coursewareUpdates[j].DownloadPath, coursewareUpdates[j].DownloadUri, coursewareUpdates[j].RetryCount);
                            }
                            find = null;
                            break;
                        }
                    ...
                    if (find != null)
                    {
                        ResourceName resourceName = new ResourceName(find.ResourceName.Name, find.ResourceName.Variant);
                        if (m_ResourceManager.m_ReadWriteResourceInfos.ContainsKey(resourceName))
                        {
                            if (File.Exists(find.DownloadPath))
                            {
                                File.Delete(find.DownloadPath);
                            }
                            m_ResourceManager.m_ReadWriteResourceInfos.Remove(resourceName);
                        }
                        //else
                        //    throw new GameFrameworkException(Utility.Text.Format("ResourceName '{0}' is invilad.", resourceName.FullName));
                    }
                }
                GenerateReadWriteList();
                m_UpdateAllowed = true;
                m_UpdateComplete = false;
                m_IsContainCourseware = true;
            }
            ...
            private void OnDownloadSuccess(object sender, DownloadSuccessEventArgs e)
            {
                ...
                if (m_ResourceManager.m_ResourceInfos.ContainsKey(updateInfo.ResourceName))
                {
                    ...
                    m_ResourceManager.m_ResourceInfos.Remove(updateInfo.ResourceName);
                }
                ...
                if (m_ResourceManager.m_ReadWriteResourceInfos.ContainsKey(updateInfo.ResourceName))
                {
                    ...
                    m_ResourceManager.m_ReadWriteResourceInfos.Remove(updateInfo.ResourceName);
                }
