...
public class KinectManager : MonoBehaviour
{
    ...
    /// <summary>
    /// true:需要重启，false：不用重启
    /// </summary>
    /// <returns></returns>
    public bool IsNeedRestart()
    {
        var kinectSensor = Windows.Kinect.KinectSensor.GetDefault();
        if (kinectSensor == null)
            return false;
        //Debug.Log("是否需要重启：" + (kinectSensor.IsOpen ? "opened" : "closed") + ", available: " + kinectSensor.IsAvailable);
        return !kinectSensor.IsOpen && !kinectSensor.IsAvailable;
    }

    public void NeedReset()
    {
        bool bNeedRestart = false;
        sensorInterfaces = KinectInterop.InitSensorInterfaces(false, ref bNeedRestart);
        if (bNeedRestart)
        {
            // set graphics shader level
            KinectInterop.SetGraphicsShaderLevel(SystemInfo.graphicsShaderLevel);
